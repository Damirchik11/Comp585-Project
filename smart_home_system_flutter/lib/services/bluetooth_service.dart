...
class BluetoothService {
  ...
  Future<void> startScan({Duration timeout = const Duration(seconds: 10)}) async {
    ...
    // Manual timeout is built-in to startScan in 2.0.2+ (it was re-added or handled differs, but sticking to plan: actually 2.0.0 removed it, then it might have come back or we should use logic)
    // Wait, checking 2.0.2 docs... "timeout" is actually available in startScan again or it's simply a duration.
    // Let's implement it the safe way: verify if 2.0.2 has timeout. Most migration guides say it behaves differently.
